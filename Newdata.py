...
config_path = "config/CycPeptMP_no_moe.json"
...
for i in range(len(new_data)):
    if (
        utils_function.canonicalize_smiles(new_data.iloc[i]["SMILES"])
        in old_data["SMILES"].to_list()
    ):
        print(
            f"Your peptide: {
                i} ({new_data.iloc[i]['ID_org']}) is already in the database."
        )
...
utils_function.enumerate_smiles(
    new_data, config, "data/new_data/enum_smiles.csv")
...
generate_conformation.generate_peptide_conformation(
    config, df_enu, "sdf/new_data/peptide.sdf"
)
...
generate_conformation.generate_monomer_conformation(
    config, df_monomer, "sdf/new_data/monomer.sdf"
)
...
calculate_descriptors.calc_rdkit_descriptors(
    new_data["SMILES"].tolist(), "desc/new_data/peptide_rdkit.csv"
)
...
# monomer
calculate_descriptors.calc_rdkit_descriptors(
    df_monomer["SMILES"].tolist(), "desc/new_data/monomer_rdkit.csv"
)
...
calculate_descriptors.calc_mordred_2Ddescriptors(
    new_data["SMILES"].tolist(), "desc/new_data/peptide_mordred_2D.csv"
)
...
# monomer
calculate_descriptors.calc_mordred_2Ddescriptors(
    df_monomer["SMILES"].tolist(), "desc/new_data/monomer_mordred_2D.csv"
)
...
calculate_descriptors.calc_mordred_3Ddescriptors(
    mols, "desc/new_data/peptide_mordred_3D.csv"
)
...
calculate_descriptors.calc_mordred_3Ddescriptors(
    mols, "desc/new_data/monomer_mordred_3D.csv"
)
...
calculate_descriptors.merge_descriptors(
    config, "desc/new_data/", "data/new_data/")
...
generate_atom_input.generate_atom_input(
    config, new_data, df_enu, mols, folder_path, set_name
)
...
generate_monomer_input.generate_monomer_input(
    config, new_data, df_mono_2D, df_mono_3D, folder_path, set_name
)
...
generate_peptide_input.generate_peptide_input(
    config, new_data, df_enu, df_pep_2D, df_pep_3D, folder_path, set_name
)
...
# Import input
set_name = "new"
dataset_new = model_utils.load_dataset(
    "model/input/new_data/", MODEL_TYPE, REPLICA_NUM, set_name
)
...
for cv in range(3):
    ...
    model = model_utils.create_model(
        best_trial, DEVICE, config["model"]["use_auxiliary"]
    )
    ...
    dataloader_now = torch.utils.data.DataLoader(
        dataset_new, batch_size=batch_size, shuffle=False
    )
    ...
    now_pred.to_csv(
        f"predicted/new_data/{MODEL_TYPE}-{REPLICA_NUM}/{set_name}_cv{cv}.csv"
    )
...
pred_cv0 = pd.read_csv(
    f"predicted/new_data/{MODEL_TYPE}-{REPLICA_NUM}/{set_name}_cv0.csv"
)
pred_cv1 = pd.read_csv(
    f"predicted/new_data/{MODEL_TYPE}-{REPLICA_NUM}/{set_name}_cv1.csv"
)
pred_cv2 = pd.read_csv(
    f"predicted/new_data/{MODEL_TYPE}-{REPLICA_NUM}/{set_name}_cv2.csv"
)
